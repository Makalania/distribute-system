...
//	fmt.Printf("share prefixlength %v\n",i, a, b)
	return i
}
...
func (id ID) BetterChoice(first ID, second ID) bool {
	...
	sharedLength := SharedPrefixLength(first, second)
	if sharedLength == DIGITS {
		return false;
	}
	if first[sharedLength] > id[sharedLength] {
		if second[sharedLength] >= id[sharedLength] {
			return first[sharedLength] < second[sharedLength]
		} else {
			return true;
		}
	} else if(first[sharedLength] < id[sharedLength]) {
		if(second[sharedLength] < id[sharedLength]) {
			return first[sharedLength] < second[sharedLength]
		} else {
			return false;
...
func (id ID) Closer(first ID, second ID) bool {
	...
	CompF := big.NewInt(0)
	CompF = CompF.Sub(BigF, BigId)
	CompF = CompF.Abs(CompF)		

	CompS := big.NewInt(0)
	CompS = CompS.Sub(BigS, BigId)
	CompS = CompS.Abs(CompS)

	if CompF.Cmp(CompS) < 0 {
		return true
	} else {
		return false
	}
